...
def random_crop(img_path='1.jpg'):
    h_select = random.randint(15,100)
    w_select = random.randint(15,100)

    occ_img = Image.open(img_path).convert('RGB')
    h_, w_ = occ_img.height, occ_img.width

    w_begin = random.randint(w_select, w_-w_select)
    h_begin = random.randint(h_select, h_-h_select)

    crop_region = occ_img.crop((w_begin,h_begin,w_begin+w_select,h_begin+h_select))
    return crop_region
...
class RandomErasing_Crop_Image(object):
    def __init__(self, EPSILON=0.5, root=None):
        self.EPSILON = EPSILON
        self.root = root
        self.occ_imgs = os.listdir(self.root)
    ...
    def __call__(self, img):
        ...
        path = os.path.join(self.root, occ_img)

        occ_img = random_crop(img_path=path)
