...
class TransReID(nn.Module):
    ...
    def forward_features(self, x, camera_id, view_id):
        ...
        if self.cam_num > 0 and self.view_num > 0:
            ...
            x = x + self.pos_embed  # self.pos_embed [1,129,768]
...
class Attention_part_global(nn.Module):
    def __init__(self, dim=768, num_heads=2, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0.):
        super().__init__()
        self.num_heads = num_heads
        head_dim = dim // num_heads
        ...
        self.q = nn.Linear(dim, dim, bias=qkv_bias)  # 3072->3072
    ...
    def forward(self, part_feature, global_feature):
        part_feature=part_feature.unsqueeze(dim=1)
        B, N, C = part_feature.shape  # 32,1,3072

        q = self.q(part_feature).reshape(B, N, 1, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)

        kv = self.kv(global_feature).reshape(B, N, 2, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        k, v = kv[0], kv[1]  # make torchscript happy (cannot use tensor as tuple) #k or v =[32,8,8,384]

        attn = (q @ k.transpose(-2, -1)) * self.scale  # [1,32,8,1,8]
        attn = attn.softmax(dim=-1)
        attn = self.attn_drop(attn)  # [1,32,8,1,8]

        recons = (attn @ v).transpose(1, 2).reshape(B, N, C)  # [32,1,3072]
...
class Attention_part_global_new(nn.Module):
    def __init__(self, dim=16, num_heads=2, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0.):
        super().__init__()
        self.num_heads = num_heads
        head_dim = dim // num_heads
...
class Block_zx(nn.Module):

    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm):
        super().__init__()
        self.norm1 = norm_layer(dim)
        self.attn = Attention_part_global_new(
            dim, num_heads=num_heads, qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop)
        # NOTE: drop path for stochastic depth, we shall see if this is better than dropout here
        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()
        self.norm2 = norm_layer(dim)
    ...
    def forward(self, x, x_global):
        x = x + self.drop_path(self.attn(self.norm1(x),self.norm2(x_global)))
        x = x + self.drop_path(self.mlp(self.norm3(x)))
        return x

class fbm_qkv(nn.Module):

    def __init__(self, emded_dim= 768, dim=4*4, num_heads=2, mlp_ratio=1., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm):
        super().__init__()
        self.norm1 = norm_layer(dim)
        self.attn = Attention_part_global_new(dim=dim, num_heads=num_heads, qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop)
        # NOTE: drop path for stochastic depth, we shall see if this is better than dropout here
        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()
        self.norm2 = norm_layer(dim)
        mlp_hidden_dim = int(dim * mlp_ratio)
        ...
        # zx 2023-5-15
        self.num_patches = int(emded_dim/(4*4))  # 4*4
        self.pos_embed = nn.Parameter(torch.zeros(1, self.num_patches, 4*4))
    ...
    def forward(self, x, x_global):
        B, L = x.shape
        x = x.view(B, self.num_patches, 4*4)
        x_global = x_global.view(B, self.num_patches, 4*4)
        x = x + self.pos_embed
        x = self.pos_drop(x)
        x_global = x_global + self.pos_embed
        x_global = self.pos_drop(x_global)
        f = self.Block(x, x_global)
        f = f.view(B, -1)
        return f
