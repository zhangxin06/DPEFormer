...
def do_train_DPEFormer(cfg,
    ...
    for epoch in range(1, epochs + 1):
        ...
        for n_iter, (img, vid, target_cam, target_view, _) in enumerate(train_loader):
            ...
            score, feat1_bn, feat2_bn, feat1_new, feat2_new = model(img, labels=target, memory_features=memory.features)
            momentum_contrast = memory(feat1_bn, target) + 0.3 * memory(feat2_bn, target, self_momentum=1)
            loss = loss_fn(score[0:4], None,target) + 2*loss_fn(score[4:6],None,target)+ loss_fn(score[6:14],None,target)

            (momentum_contrast+loss).backward()
            optimizer.step()
            ...
            if (n_iter + 1) % log_period == 0:
                logger.info(
                    "Epoch[{}] Iteration[{}/{}] Loss: {:.3f}; contrast_loss: {:.3f}; Acc: {:.3f}, Base Lr: {:.2e}"
                    .format(epoch, (n_iter + 1), len(train_loader),
                            loss_meter.avg, loss_momentum_contrast.avg, acc_meter.avg,
